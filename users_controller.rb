def authorize
  ...
  unless params[:code]
    ...
    if hash['code'] &&
        hash['code'] == 400
      redirect_to("#{Rails.application.config.front_end_url}/my-stats?status=error&code=1003")
      return
    end
    ...
    if current_user.ig_users.exists?(params[:ig_user][:id])
      redirect_to("#{Rails.application.config.front_end_url}/my-stats?status=error&code=1001")
      return
    end
    ...
    if IgUser.exists?(params[:ig_user][:id])
      ...
      if ig_user.brand &&
          ig_user.brand_id != current_user.id
        redirect_to("#{Rails.application.config.front_end_url}/my-stats?status=error&code=1002")
        return
      ...
      if not current_user.ig_user
        current_user.ig_user = ig_user
      end
    end
    ...
    redirect_to("#{Rails.application.config.front_end_url}/my-stats?status=success&id=#{ig_user.id}")
  end
end